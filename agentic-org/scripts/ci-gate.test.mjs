...
import { GREEN, GateError, PENDING, RED, parseArgs, verdict, waitForGreen } from './ci-gate.mjs';
...
test('a pull_request duplicate GitHub never let start does not veto a green push run', () => {
  // Observed live on 2026-09-06: the PR this workflow opens is authored by
  // github-actions[bot], whose runs are parked behind "Approve and run", so the
  // duplicate concludes action_required while the push run is green.
...
test('any other non-success duplicate still vetoes', () => {
  for (const conclusion of ['failure', 'cancelled', 'timed_out', 'startup_failure', 'neutral', 'stale'])
