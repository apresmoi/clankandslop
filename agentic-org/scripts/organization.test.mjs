...
test('root declaration keeps Moltnet durable, authenticated, direct and secret-backed', () => {
  ...
  assert.throws(() => validateRootDeclaration(root.replace('federation: [clank-observer, clank-luna]', 'federation: [legacy-peer]')), /federate only to the observer and Luna pairings/);
  assert.throws(() => validateRootDeclaration(root.replace(', research-sensor, klaxon', ', klaxon')), /research sensor local identity/);
  assert.throws(() => validateRootDeclaration(root.replace('members: [clank-luna:luna, gatherer, klaxon', 'members: [clank-luna:luna, klaxon')), /assignment kickoff participant/);
});
