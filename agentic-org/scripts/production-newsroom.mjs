...
export async function recordAssignment(args){
  ...
  await converge(location(args.edition,'assignments',sha(args.event_key).slice(7,39)),{version:'clank.assignments.v1',...args});
...
export async function fileArticle(args){
  ...
  await converge(location(args.edition,'filings',`${assignment.id}/${resolvedArticle.revision}`),filing);
...
export async function reviewArticle(args){
  ...
  await converge(location(args.edition,'verdicts',`${args.article_id}/${args.revision}`),review);
  if(args.verdict==='PASS'){const{assignment_ref:_,...article}=filing;await converge(location(args.edition,'articles',args.article_id),article);await converge(location(args.edition,'reviews',args.article_id),review);}
...
export async function fileDesk(args){
  ...
  await converge(location(args.edition,'desk',args.name),object(args.document));
...
export async function composeEdition(args){
  ...
  for(const page of args.pages)await converge(location(args.edition,'pages',page.name),page.document);for(const map of args.maps??[]){if(!component.test(map.name))throw new Error(`maps[].name ${JSON.stringify(map.name)} must be 1-128 lowercase alphanumeric/hyphen characters, starting with a letter or digit`);await converge(location(args.edition,'maps',map.name),object(map.document));}
