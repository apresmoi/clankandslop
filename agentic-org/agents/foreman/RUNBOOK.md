...
them: `used_by_agent` is my name; a `review.fragment_missing` entry needs captured
support or a faithful, honestly identified paraphrase of supplied findings,
never an invented excerpt. Follow WRITING.md for missing support. Only unused entries outside the assignment's `evidence_refs` may come out and the rest renumber; required assignment evidence
remains a blocker. `key_numbers` there are candidates in
