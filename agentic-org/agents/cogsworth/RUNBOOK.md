...
This file preserves the task mechanics and examples for Cogsworth. The always-on boundaries live in AGENTS.md; identity lives in SOUL.md. Read this runbook when a wake requires the detailed article, dissent, art or research workflow.
...
name; a `review.fragment_missing` entry needs captured support or a faithful,
honestly identified paraphrase of supplied findings, never an invented excerpt.
Follow WRITING.md for missing support. Only unused entries outside the assignment's `evidence_refs` may come
out, with the rest renumbered; required assignment evidence remains a blocker. `key_numbers` and `next_update_utc` in there are stock, not
